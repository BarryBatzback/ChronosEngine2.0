...
class ArchitectureParser:
    ...
    def __init__(self, root_dir: Optional[Path] = None):
        ...
        self.default_registry = {
            "metadata": {
                "version": "2.0",
                "game_setting": "Eurasia 800-900 AD"
            },
            "materials": {},
            "meshes": {}
        }
    ...
    def parse_text_file(self, file_path: Path) -> Optional[Dict[str, Any]]:
        ...
        sub_type = sub_type_match.group(1).strip().lower() if sub_type_match else "general"
        ...
        params = {}
        param_section = re.search(r"ПАРАМЕТРЫ ДЛЯ GENERATOR:(.*?)(?=\n\[|\n\n|$)", content, re.DOTALL)
        if param_section:
        ...
        if dim_match:
            ...
            if len(nums) >= 3:
                dimensions = {"length": float(nums[0]), "width": float(nums[1]), "height": float(nums[2])}
    ...
    def process_docs_folder(self):
        ...
        return f"Парсинг завершен. Всего обработано и добавлено ассетов: {processed_count}"
