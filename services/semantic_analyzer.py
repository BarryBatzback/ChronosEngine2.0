...
class SemanticAnalyzer:
    def __init__(self, llm_service=None):
        ...
        self.object_rules = {
            'sword': {
                'keywords': ['sword', 'blade', 'weapon', 'katana', 'rapier', 'claymore'],
                'components': [
                    {'name': 'blade', 'type': 'extruded', 'dimensions': [0.1, 0.02, 1.2], 'material': 'metal'},
                    {'name': 'guard', 'type': 'box', 'dimensions': [0.25, 0.06, 0.04], 'material': 'metal'},
                    {'name': 'handle', 'type': 'cylinder', 'dimensions': [0.04, 0.04, 0.4], 'material': 'wood'},
                    {'name': 'pommel', 'type': 'sphere', 'dimensions': [0.06, 0.06, 0.06], 'material': 'metal'}
                ]
            },
            'table': {
                'keywords': ['table', 'desk', 'furniture'],
                'components': [
                    {'name': 'top', 'type': 'box', 'dimensions': [1.2, 0.05, 0.8], 'material': 'wood'},
                    {'name': 'leg1', 'type': 'cylinder', 'dimensions': [0.05, 0.05, 0.9], 'material': 'wood'},
                    {'name': 'leg2', 'type': 'cylinder', 'dimensions': [0.05, 0.05, 0.9], 'material': 'wood'},
                    {'name': 'leg3', 'type': 'cylinder', 'dimensions': [0.05, 0.05, 0.9], 'material': 'wood'},
                    {'name': 'leg4', 'type': 'cylinder', 'dimensions': [0.05, 0.05, 0.9], 'material': 'wood'}
                ]
            },
            'tree': {
                'keywords': ['tree', 'plant', 'forest', 'oak', 'pine'],
                'components': [
                    {'name': 'trunk', 'type': 'cylinder', 'dimensions': [0.2, 0.2, 2.0], 'material': 'wood'},
                    {'name': 'foliage', 'type': 'sphere', 'dimensions': [1.0, 1.0, 1.0], 'material': 'leaves'}
                ]
            }
        }
    ...
    def _get_components(self, object_type: str, prompt: str) -> List[Dict]:
        ...
        return [{"name": "main", "type": "cube", "dimensions": [1, 1, 1], "material": "default"}]

    def _adjust_materials(self, object_type: str, prompt: str) -> Dict:
        materials = {}
        # Загрузка базовых материалов
        if object_type == 'sword':
            materials = {'metal': self.materials['metal'].copy(), 'wood': self.materials['wood'].copy()}
        elif object_type == 'tree':
            materials = {'wood': self.materials['wood'].copy(), 'leaves': self.materials['leaves'].copy()}
        else:
            materials = {'default': self.materials['default'].copy()}

        # Цветовые модификации
        if 'gold' in prompt:
            if 'metal' in materials: materials['metal'] = self.materials['gold'].copy()
        if 'red' in prompt:
            for mat in materials.values(): mat['color'] = [0.8, 0.0, 0.0]
        if 'black' in prompt:
            for mat in materials.values(): mat['color'] = [0.02, 0.02, 0.02]
