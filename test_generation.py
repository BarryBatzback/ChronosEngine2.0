...
async def test_palisade_generation():
    ...
    if not registry_path.exists():
        logging.error("❌ Файл базы данных не найден! Сначала запусти services/architecture_parser.py")
        return
...
"""
    ...
    logging.info("🧠 Шаг 3: Формирование промпта завершено. Инициализация Orchestrator...")
    ...
    try:
        ...
        result = await orchestrator.generate_asset_workflow(
            prompt=prompt, 
            category=asset_data.get("sub_category", "defense")
        )
