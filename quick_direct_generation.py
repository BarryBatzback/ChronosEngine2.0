...
async def direct_execute(self, task: Dict[str, Any], session_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"📐 Direct structure: {structure.get('object_type')} with {len(structure.get('components', []))} components")
        ...
        if result.get('status') == 'ok':
            ...
            return await self._fix_and_retry(code, result.get('message', ''), structure, user_prompt)
...
def _prompt_to_structure(self, prompt: str) -> Dict:
    ...
    if any(word in prompt_lower for word in ['sword', 'blade', 'weapon']):
        return {
            "object_type": "sword",
            "components": [
                {"name": "blade", "type": "extruded", "dimensions": [0.1, 0.02, 1.2], "position": [0, 0, 0.6], "material": "metal"},
                {"name": "guard", "type": "box", "dimensions": [0.25, 0.06, 0.04], "position": [0, 0, 0.1], "material": "gold"},
                {"name": "handle", "type": "cylinder", "dimensions": [0.04, 0.04, 0.4], "position": [0, 0, -0.3], "material": "wood"},
                {"name": "pommel", "type": "sphere", "dimensions": [0.06, 0.06, 0.06], "position": [0, 0, -0.5], "material": "gold"}
            ],
            "materials": {
        ...
        return {
            "object_type": "object",
            "components": [
                {"name": "main", "type": "cube", "dimensions": [1, 1, 1], "position": [0, 0, 0], "material": "default"}
            ]
        }
